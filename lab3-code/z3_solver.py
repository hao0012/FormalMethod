...
F = And(And(And(P, Not(S)), R), And(R, Or(~P, And(S, Not(Q)))))
print("======Exercise 1-4=======")
pretty_print(F)
print("solution: ")
solve(F)
...
print("solution: ")
solve(F)
F = And(F, Not(And(Not(R), And(P, Q))))
solve(F)
F = And(F, Not(And(P, And(Q, R))))
solve(F)
F = And(F, Not(And(R, And(Not(P), Not(Q)))))
solve(F)
F = And(F, Not(And(R, And(P, Not(Q)))))
solve(F)
F = And(F, Not(And(Not(P), And(R, Q))))
solve(F)
print("=========================\n")
...
# To wrap these together, we have this src:
solver = Solver()
