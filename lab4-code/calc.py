...
def pp_exp_format(e: Exp) -> str:
    match e:
        case ExpVar(var):
            return var
        case ExpBop(left, right, bop):
            return f"{pp_exp_format(left)} {bop} {pp_exp_format(right)}"
def pp_exp(e: Exp):
    print(pp_exp_format(e))

def pp_stm_format(s: Stm) -> str:
    match s:
        case StmAssign(var, exp):
            return f"{var} = {pp_exp_format(exp)}"

def pp_stm(s: Stm):
    print(pp_stm_format(s))

def pp_func_format(f: Function) -> str:
    match f:
