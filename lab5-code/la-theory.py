...
# Integer domain. For instance, this src
x, y = Ints('x y')
...
print("solution in domain R:")
print_model(res, model)

x, y = Ints('x, y')
cons = [x + y == 8, x - y == 1]
res, model = check_cons(cons)
print("solution in domain Z:")
print_model(res, model)
print()
...
def check_zero_la(l):
    ...
    for i in range(len(l)):
        cons_exp.append(l[i] * vars[i])
    cons_sum2 = [sum(cons_exp) == 0]
    # check these constraints:
    res, model = check_cons(cons_0_or_1 + cons_sum + cons_sum2)
    if res == sat:
        print(model)
        return True
    return False
...
x, p, q = Ints('x p q')
res, model = check_cons([x * q == p, p == 2, x == q])
print("\nresult for Exercise 8:")
if res == sat:
    print(model)
else:
    print(res)
